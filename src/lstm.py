...
from model_utils import GlobalMaxPooling, CommentsLSTMEncoder
...
def train_with_cv(train):
    ...
    comment_encoder = CommentsLSTMEncoder(out_size=64, n_tokens=len(tokens), PAD_IX=PAD_IX).cuda()
...
def main(train, test, sub, cv=True, filename='baseline'):
    if cv:
        train_with_cv(train)
    else:
...
if __name__ == "__main__":
    ...
    main(train, test, sub, filename=f'lstm_{num_epochs}')